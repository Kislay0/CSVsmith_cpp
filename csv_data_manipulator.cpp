...
void CSVData::convert_date_format(const std::string &old_format, const std::string &new_format, int row, int column)
{
    ...
    const string& old_date = row_data.at(column);

    std::tm od{}; // zero-initialize
    std::istringstream iss(old_date);
    iss >> std::get_time(&od, old_format.c_str());
    if (iss.fail()) {
        cerr << "Failed to parse date '" << old_date
             << "' with format '" << old_format << "'" << endl;
        return;
    }

    std::ostringstream oss;
    oss << std::put_time(&od, new_format.c_str());
    m_data.at(row).at(column) = oss.str();
